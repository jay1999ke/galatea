...
def train(alpha=0.05):
    ...
    for i in range(150):
        ...
        torch.save(model.W,"weights/boltzmann/W24.pt")
...
def test():
    ...
    # model = train()
    model = Boltzmann()
    model.W = torch.load("weights/boltzmann/W24.pt")
    ...
    for k in range(10):
        ...
        for j in range(5):
            for i in range(model.total_units):
                ...
                plt.imshow(model.state[:model.visible_units].view(4,4).numpy())
                plt.pause(0.0001)
